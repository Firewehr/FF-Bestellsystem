...
try {
    ...
    $sql1 = "SELECT MAX(`positionen`.`type`) AS type, MAX(tische.tischname) AS tischname, bestellungen.tischnummer, "
        . "{$batchKeyExpr} AS batch_key, COUNT(*) AS cnt "
        . "FROM bestellungen, positionen, tische "
        . "WHERE tische.tischnummer=bestellungen.tischnummer AND bestellungen.position=positionen.rowid AND `delete`=0 AND `kueche`=0 "
        . "GROUP BY bestellungen.tischnummer, {$batchKeyExpr} "
        . "ORDER BY MIN(bestellungen.zeitstempel) ASC LIMIT 50";
    ...
    while ($row1 = mysqli_fetch_assoc($result1)) {
        ...
        $batch_key = (string)($row1['batch_key'] ?? '');
        ...
        while ($row = mysqli_fetch_assoc($result2)) {
            ...
            //echo '<td>';
            //echo '<input id="' . $row['rowid'] . '" style="background:' . $Colour . ';" onclick="Summe+=' . $row['betrag'] . ';document.getElementById(' . $row['rowid'] . ').style.backgroundColor = \'#00CC00\';document.getElementById(sum).value=1;" type="button" value="' . htmlspecialchars((string)($row['Positionsname'] ?? ''), ENT_QUOTES, 'UTF-8') . '" />';
            ////echo '<button class="ui-btn ui-corner-all" id="' . $row['rowid'] . '" style="background:' . $Colour . ';">' . htmlspecialchars((string)($row['Positionsname'] ?? ''), ENT_QUOTES, 'UTF-8') . '"</button>';
            //echo '</td>';
            ...
            if ($row['zeitKueche'] == '0000-00-00 00:00:00') {
                ...
                echo '<a style="background-color:' . $color . '" href="#" class="ui-btn ui-icon-delete ui-btn-icon-left" onclick="bestellungLoeschen(' . $row['rowid'] . ',' . $Tischnummer . ');"' . '>' . htmlspecialchars((string)($row['Positionsname'] ?? ''), ENT_QUOTES, 'UTF-8') . ' Bestellt: ' . date("H:i", strtotime($row['zeitstempel'])) . ' Fertig: ' . date("H:i", strtotime($row['zeitKueche'])) . '</a>';
