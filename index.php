...
<head>
    ...
    <style>
        ...
        .menu-card .list-group-item-action {
            color: #b91c1c;
        }
        ...
        #DruckzielContent,
        #DruckzielHistoryContent,
        #adminPage,
        #financePage,
        #listTische,
        #listTischBestellungen,
        #Direktverkauf,
        #MitarbeiterVerpflegungPage,
        #myOrdersPage,
        #bestellHistoryPage,
        #Kuechenansicht,
        #Schankansicht {
        ...
        .ff-terminal-mode body.app-page {
            font-size: 0.80rem !important;
        }

        .ff-terminal-mode .app-content {
            max-width: 920px !important;
            padding: 0.5rem 0.45rem !important;
        }

        .ff-terminal-mode .navbar.app-navbar .btn,
        .ff-terminal-mode .navbar.app-navbar .navbar-brand {
            font-size: 0.80rem !important;
            padding-top: 0.18rem;
        ...
        .ff-terminal-mode .posWrap>button[id^="btn-pos-"] {
        ...
        .ff-terminal-mode .table th {
            padding: 0.28rem 0.3rem !important;
            font-size: 0.82rem !important;
        }

        .ff-terminal-mode .kueche-orders-flow {
            gap: 0.45rem !important;
        }

        .ff-terminal-mode .kueche-bestellung-wrap {
        ...
        .ff-terminal-mode .ff-kueche-dv-bon-wrap {
            padding: 0.35rem 0.55rem 0.4rem !important;
            border-width: 2px !important;
        }

        .ff-terminal-mode .ff-kueche-dv-bon-wrap .ff-kueche-dv-bon-label {
            font-size: 0.64rem !important;
        }

        .ff-terminal-mode .ff-kueche-dv-bon-wrap .ff-kueche-dv-bon-nr {
            font-size: clamp(0.92rem, 2.2vw, 1.35rem) !important;
        }
...
<body class="app-page">
    ...
    <script>
        (function() {
            ...
            window.ffApplyTerminalMode = function(enabled, persist) {
                ...
                if (persist) {
                    try {
                        localStorage.setItem('ff_terminal_mode', window.ffTerminalModeEnabled ? '1' : '0');
                    } catch (e) {}
                }
    ...
    <!-- IndexedDB -->
    <script id="ffPositionsSeedJson" type="application/json">
        <?php
        $ffPosSeed = [];
    ...
    <script>
        (function() {
            ...
            request.onerror = function(e) {
                console.error("IndexedDB error:", e && e.target && e.target.error);
            };
            request.onupgradeneeded = function() {
                var db = request.result;
                var store = db.createObjectStore("positionen", {
                    keyPath: "rowid"
                });
    ...
    <!-- Tisch-Auswahl Modal: QR-Scan oder Alle Tische -->
    <div class="modal fade" id="ffTischSelectModal" tabindex="-1" aria-labelledby="ffTischSelectModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ffTischSelectModalLabel">Tisch wählen</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Schließen"></button>
                </div>
                <div class="modal-body">
                    <p class="small text-muted">Bitte wählen Sie eine Option, um fortzufahren.</p>
                    <button type="button" class="btn btn-primary w-100 mb-2" id="ffTischScanBtn" onclick="startTischQrScan();">Tisch per QRCode Scannen</button>
                    <button type="button" class="btn btn-outline-secondary w-100" id="ffTischAllBtn" onclick="closeTischSelectAndLoadAll();">Alle Tische</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- QR-Scanner Modal -->
    <div class="modal fade" id="ffTischScanModal" tabindex="-1" aria-labelledby="ffTischScanModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-fullscreen modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ffTischScanModalLabel">QR-Code Scanner</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Schließen" onclick="stopTischQrScan();"></button>
                </div>
                <div class="modal-body p-0">
                    <div id="ffTischScanRegion" style="width:100%;height:60vh;display:flex;align-items:center;justify-content:center;"></div>
                    <div id="ffTischScanFallback" style="padding:1rem; display:none;">
                        <p class="small text-muted">Scanner nicht verfügbar? Versuche eine Bilddatei hochzuladen oder gib die Tischnummer manuell ein.</p>
                        <div class="mb-2">
                            <input type="file" accept="image/*" id="ffTischScanFileInput" class="form-control">
                            <button type="button" class="btn btn-primary w-100 mt-2" onclick="startTischQrFromImage();">QR aus Bild lesen</button>
                        </div>
                        <div>
                            <label class="form-label">Tischnummer manuell eingeben</label>
                            <div class="input-group">
                                <input type="number" id="ffTischScanManualInput" class="form-control" placeholder="Tischnummer">
                                <button type="button" class="btn btn-outline-secondary" onclick="_handleManualTischEntry();">Öffnen</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary w-100" data-bs-dismiss="modal" onclick="stopTischQrScan();">Abbrechen</button>
                </div>
            </div>
        </div>
    </div>
